...
MySQL.prototype.buildOrderBy = function(model, order, aliases) {
  ...
  for (var i = 0, n = order.length; i < n; i++) {
    ...
    if (t.length === 1) {
      ...
      var key = t[0];
      if (key.indexOf('.') > -1) {
        const [subModel, subModelProperty] = key.split('.');
        ...
        if (relations && relations[subModel]) {
          ...
          alias = Object.values(aliases).find(aliasValue => deleteAliasQuotes(aliasValue) == subModel);
        }
        if (alias) {
          clauses.push(sign + alias + '.' + subModelProperty + ' ' + t[1]);
          columnNames.push(alias + '.' + subModelProperty +
            ' as ' + deleteAliasQuotes(alias) + '_orderBy' + subModelProperty);
        }
      } else {
